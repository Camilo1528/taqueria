...
async function cargarMesas() {
    const res = await fetch('/tables/status', { headers: {'x-api-key': '123456'} });
...
function abrirMenu(id, num, activa) {
    if (!activa) {
        Swal.fire({icon: 'warning', title: 'Mesa Cerrada', text: 'La mesa debe ser abierta primero en la caja principal.'});
        return;
    }
    mesaId = id;
